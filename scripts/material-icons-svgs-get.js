...
async function verifyAndOptimize(filePath) {
    const contents = await readFile(filePath, 'utf8');
    if (!contents.startsWith("<svg ")) {
        console.error("Not svg in: " + filePath);
        await rename(filePath, filePath.replace(".svg", ".error.html"))
    } else {
        await _optimizeSVG(contents)
    }

    async function _optimizeSVG(contents) {
        const {data, info} = await svgoInstance.optimize(contents, {path: filePath});
